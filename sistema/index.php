...
<body>
	...
	$query_ganancia = mysqli_query($conection, "SELECT SUM(monto) as ganancia FROM ganancias Where estatus = 1");
	$result_ganancia = mysqli_num_rows($query_ganancia);
	if($result_ganancia > 0){
	$ganancia = mysqli_fetch_assoc($query_ganancia);
	}

	$formatoMoneda_ganancia = number_format($ganancia['ganancia'], 2);
...
<form action="buscar_monto_venta.php" method="get" class="form_search_date">
<label>De: </label>
...
<a href="#"><i class="fa-solid fa-money-bill"></i>
				<p>
					...
					<span><?php echo '₡'.$formatoMoneda_ganancia; ?></span>
				</p>
				</a>
		</div>
		</div>
