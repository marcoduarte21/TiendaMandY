...
<form action="buscar_ventaCredito.php" method="get" class="form_search_date">
<label>De: </label>
...
$sql_register = mysqli_query($conection, "SELECT COUNT(*) as total_registro FROM factura WHERE estatus != 10 AND id_formapago = 2");
...
WHERE f.estatus != 10 AND id_formapago = 2
...
if ($result > 0) {
    ...
    while ($data = mysqli_fetch_array($query)) {
        ...
        if ($data["estatus"] == 1) {
            $estatus = '<span class="pagada">Generada</span>';
        } else {
        ...
        <tr id="row_<?php echo $data["nofactura"]; ?>">
            ...
            <div>
            <button class="btn_view view_factura_credito" type="button" cl="<?php echo $data["codcliente"]; ?>" f="<?php echo $data["nofactura"]; ?>"><i class="fa-solid fa-eye"></i></button>
            </div>
